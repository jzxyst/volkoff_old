...
/*! @par include */
#include "stdafx.h"
#include	"DT_Valfirle.h"	/* For DynamicTracer-TestPoint */
#include "Valfirle.h"
#include "Player.h"
...
Valfirle::Valfirle()
    : mWeaponLevel  ( 1 )
{
	__DtTestPoint( __DtFunc_Valfirle, __DtStep_0 )
	...
	for(int i = 0;i < 10;++i)
	{
		__DtTestPoint(__DtFunc_Valfirle, __DtStep_1)
		this->m_nNum[i] = 0;
	}

    // 攻撃対象グループは敵側
    setAttackedGroup( ATKGROUP_ENEMY );
	__DtTestPoint( __DtFunc_Valfirle, __DtStep_2 )
}
...
Valfirle::~Valfirle()
{
	__DtTestPoint( __DtFunc_Valfirle, __DtStep_3 )
	this->Release();
	__DtTestPoint( __DtFunc_Valfirle, __DtStep_4 )
}
...
bool Valfirle::Initialize( const CreateData& data_ )
{
	...
	this->mCharaPanel.setColorScale(0.8f,0.8f,0.95f);
    this->m_pszName = "Valfirle"; //一時的。後で適当な名前を代入する
...
{
	//第二段階への変更を行う
	__DtTestPoint( __DtFunc_ChangePhase, __DtStep_0 )
	if(this->mLife <= scg_nValfirleLifePhase2[GameManager::getInstance()->getRank()])
	{
		__DtTestPoint(__DtFunc_ChangePhase, __DtStep_1)
		this->m_nPhase = 1;
	}

	__DtTestPoint( __DtFunc_ChangePhase, __DtStep_2 )
	return;
}
